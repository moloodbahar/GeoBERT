...
#  epochs = 1,
#  batchsize= 32,
#  save_path= 'exBERT/oilBERToutputwandb',
#  device= [0, 1, 2, 3],
#  learning_rate= 0.0001,
#  strategy= 'exBERT',
#  config= ['exBERT/config_and_vocab/exBERT/bert_config_ori.json',
#   'exBERT/config_and_vocab/oilBERT/bert_config_oilex_s3.json'],
#  vocab= 'exBERT/config_and_vocab/oilBERT/oilvocabdic.txt',
#  pretrained_model_path= 'exBERT/modeldict',
#  pretrained_model_path_tf= None,
#  datat_path= 'exBERT/geology_petrology_corpus.pkl',
...
ap.add_argument("--epochs", required = True, type = int, help='number of training epochs')
ap.add_argument("--batchsize", required = True, type = int, help='training batchsize')
ap.add_argument("--save_path",required = True, type = str, help='path to storaget the loss table, stat_dict')
ap.add_argument('--learning_rate',required = True, type = float, help='learning rate , google use 1e-04')
ap.add_argument('--strategy',required = True, type = str, help='choose a strategy from [exBERT], [sciBERT], [bioBERT]')
ap.add_argument('--vocab',required = True, type = str, help='path to the vocab file for tokenization')
ap.add_argument('--pretrained_model_path',default = None, type = str, help='path to the pretrained_model stat_dict (torch state_dict)')
ap.add_argument('--pretrained_model_path_tf',default = None, type = str, help='path to the pretrained_model (tensorflow .ckpt)')
ap.add_argument('--datat_path',required = True, type = str, help='path to data ')
ap.add_argument('--longest_sentence', required = True, type = int, help='set the limit of the sentence lenght, recommand the same to the -dt')
ap.add_argument('--percentage', required = True, type = float, help='the percentage used for pretraining')
ap.add_argument('--sep', default = 1, type = int, help = 'sep the training process of epoch in to N ')
ap.add_argument('--warmup', default=-1, type=float, help='portion of all training itters to warmup, -1 means not using warmup')
ap.add_argument('--train_extension_only', default=True, type=bool, help='train only the extension module')
...
#         args['strategy']= 'exBERT'
        args['device']=[0, 1, 2, 3]
        args['config']=['exBERT/config_and_vocab/exBERT/bert_config_ori.json','exBERT/config_and_vocab/oilBERT/bert_config_oilex_s3.json']
        ...
        class pre_train_BertTokenizer(BertTokenizer):
            ...
            def prepare_batch(self, Train_Data,Train_Label, batch_size=args['batchsize'], longest_sentence=args['longest_sentence']):
        ...
        Train_Data,Train_Label,Val_Data,Val_Label = load_data( args['datat_path'] ,1,20000, args['percentage'], 200, load_all=False)
        print('done data preparation')
        print('data number: '+str(len(Train_Data)))
        print('data validation: '+str(len(Val_Data)))
        ...
        print('total batch number: '+str(total_batch_num))
        print('sep_range is: '+str(sep_range))
        print('all_data_num is: '+str(all_data_num))
        ...
        print('print_every_ndata: '+str(print_every_ndata), int(all_data_num/batch_size))
        ...
        try:
            for epoc in range(num_epoc):
                ...
                for sep_data_id in range(args['sep']):
                    ...
                    for batch_ind in range(int(np.floor(len(Train_Data)/batch_size))):
                        ...
                        if batch_ind>0 and batch_ind % print_every_ndata ==0:
                            print('Train Epoch: {} [{}/{} ({:.0f}%)]\ttrain_Loss: {:.5f} \tval_Loss: {:.5f} time: {:.4f} \t lr:{:.6f}'.format(
                                    ...
                                    optimizer.get_lr()[0]))
                            wandb.log({'training_batch_step':batch_ind ,'avg_train_loss': train_loss/print_every_ndata/batch_size, 'batch_size': batch_size,'Learning_rate': optimizer.get_lr()[0]})
                            train_loss = 0
                            val_loss = 0
#                             with open(args['save_path']+'/loss.pkl','wb') as f:
#                                 pickle.dump([train_los_table,val_los_table,args],f)
                    if len(device_ids)>1:
                        t.save(model.module.state_dict(),args['save_path']+'/state_dic_'+args['strategy']+'_'+str(epoc)+'_'+str(batch_size)+'_'+str(args['learning_rate']))
                    else:
                        t.save(model.state_dict(),args['save_path']+'/state_dic_'+args['strategy']+'_'+str(epoc)+'_'+str(batch_size)+'_'+str(args['learning_rate']))
#                     with open(args['save_path']+'/loss.pkl','wb') as f:
#                         pickle.dump([train_los_table,val_los_table,args],f)
                model.eval()
                with t.no_grad():
                    for batch_ind in range(int(np.floor(len(Val_Data)/batch_size))):
                        ...
                        val_loss+=val_log
                        wandb.log({'val_batch_loss': val_log})
                wandb.log({'val_loss': val_loss/(batch_ind+1), 'train_epoch': epoc})   
                print('Val_loss: '+str(val_loss/(batch_ind+1)))
                if val_loss.data < best_loss:
                    if len(device_ids)>1:
                        t.save(model.module.state_dict(),args['save_path']+'/Best_stat_dic_'+args['strategy']+str(epoc)+'_'+str(batch_size)+'_'+str(args['learning_rate']))
                    else:
                        t.save(model.state_dict(),args['save_path']+'/Best_stat_dic_'+args['strategy']+str(epoc)+'_'+str(batch_size)+'_'+str(args['learning_rate']))
